 ...
 */
fun Context.isNetworkConnected() = (getSystemService(Context.CONNECTIVITY_SERVICE) as? ConnectivityManager)?.run {
    ...
    if (Build.VERSION.SDK_INT < Build.VERSION_CODES.LOLLIPOP) {
        ...
        wifi?.isConnected  == true || mobile?.isConnected == true
    } else {
        var connected = false
        allNetworks
            .asSequence()
            ...
            .forEach { networkInfo ->
                connected = connected or (networkInfo?.isConnected == true)
            }
        connected
    }
} ?: false
